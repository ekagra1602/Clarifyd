...
export function AvatarPreview({ avatar, size = "md" }: { avatar: AvatarShape; size?: "sm" | "md" | "lg" | "xl" }) {
  ...
  return (
    ...
    >
      ...
      {hairStyle === "ponytail" && (
        <>
          <path
            ...
            strokeWidth="1.5"
          />
          <ellipse cx="60" cy="78" rx="12" ry="14" fill={hairHex} stroke="#1E293B" strokeWidth="1.2" />
        </>
      )}
      {hairStyle === "bun" && (
        <path
      ...
      {eyeStyle === "default" && (
        <>
          <circle cx="45" cy="68" r="4" fill="#1E293B" />
          <circle cx="75" cy="68" r="4" fill="#1E293B" />
        </>
      )}
      {eyeStyle === "wide" && (
        <>
          <ellipse cx="45" cy="68" rx="5" ry="4" fill="#1E293B" />
          <ellipse cx="75" cy="68" rx="5" ry="4" fill="#1E293B" />
        </>
      )}
      {eyeStyle === "narrow" && (
        <>
          <path d="M42 68 L48 68" stroke="#1E293B" strokeWidth="2.5" strokeLinecap="round" />
          <path d="M72 68 L78 68" stroke="#1E293B" strokeWidth="2.5" strokeLinecap="round" />
        </>
      )}
      {eyeStyle === "smile" && (
        <>
          <path d="M43 66 Q45 70 47 66" stroke="#1E293B" strokeWidth="1.5" fill="none" strokeLinecap="round" />
          <path d="M73 66 Q75 70 77 66" stroke="#1E293B" strokeWidth="1.5" fill="none" strokeLinecap="round" />
        </>
      )}

      {/* Mouth (simple) */}
      <path d="M50 88 Q60 94 70 88" stroke="#1E293B" strokeWidth="1.5" fill="none" strokeLinecap="round" />

      {/* Glasses */}
      {showGlasses && (
        <>
          <circle cx="45" cy="68" r="10" fill="none" stroke="#1E293B" strokeWidth="2" />
          <circle cx="75" cy="68" r="10" fill="none" stroke="#1E293B" strokeWidth="2" />
          <path d="M55 68 L65 68" stroke="#1E293B" strokeWidth="2" />
        </>
      )}
    </svg>
  );
